(() => {
  ...
  function jobCardHtml(job){ return `<article class="jobCard ${job.priority}" draggable="${state.role==='supervisor'}" data-job-id="${job.id}"><h3>${esc(job.job_name)}</h3>${job.address?`<p>${esc(job.address)}</p>`:''}${job.miss_dig_number?`<p>Miss Dig: ${esc(job.miss_dig_number)}</p>`:''}${job.job_number?`<p>D’Angelo Job #: ${esc(job.job_number)}</p>`:''}${job.notes?`<p class="notes">${esc(job.notes)}</p>`:''}</article>`; }
  function bindBoard(){
  ...
  function blankDraft(){ return {job_name:'',address:'',miss_dig_number:'',job_number:'',notes:'',customer:'',customer_phone:'',job_type:'',permit_number:'',inspector:'',priority:'non_emergency',start_date:null,end_date:null,crew_id:null}; }
